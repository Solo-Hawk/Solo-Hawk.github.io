...
function create() {
    ...
    var map = [[0, 0]
           ...
           , [(game.width / 4) * 3, (game.height / 4) * 3]
    ...
    for (var i = 0; i < boidsAmount; i++) { // deploying each friendly boid
        ...
        // Boid ship creation
        boids[i] = game.add.sprite(node[0].x + randX, node[0].y + randY, goodShips[game.rnd.between(0, 2)])
    ...
    for (var i = 0; i < hostileboids; i++) { // deploying each hostile boid
        ...
        // Boid ship creation
        hboids[i] = game.add.sprite(node[nodes - 1].x + randX, node[nodes - 1].y + randY, badShips[game.rnd.between(0, 2)])
...
function update() {
    ...
    if (respawnWave < tick) {
        ...
        var randY = game.rnd.between(-50, 50);
            ...
            if(hboids[i].alive == false){
               
                hboids[i].alive = true;
                hboids[i].exists = true;
                hboids[i].visible = true;
            }
            if(boids[i].alive == false){
    ...
    for (var i = 0; i < limit; i++) {
        ...
        var randY = game.rnd.between(-50, 50);
            if (checkOverlap(bfield[i], hboids[x]) && hboids[x].alive && boids[i].alive && tick > bfire[i]) {
                ...
                hboids[x].x = node[nodes - 1].x 
                hboids[x].y = node[nodes - 1].y
                fireCheck = true;
