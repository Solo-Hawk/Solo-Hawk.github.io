...
function createMap(game) {
    ...
    map = [
             [50, 50]
            , [((game.width / 32) * 12) - 45, 50]
            , [50, ((game.height / 20) * 8) - 45]
            , [((game.width / 32) * 12) - 45, ((game.height / 20) * 12) - 45]
            , [(game.width / 2) - 45, (game.height / 2) - 45]
            , [((game.width / 32) * 20) - 45, ((game.height / 20) * 8) - 45]
            , [game.width - 141, ((game.height / 20) * 12) - 45]
            , [((game.width / 32) * 20), game.height - 141]
...
function create() {
    ...
    for(var x = 0; x < 400; x++) {
        connector.lineStyle(2, 0xffffff, 1)
        connector.drawCircle(game.rnd.between(0, game.width),game.rnd.between(game.height, 50),game.rnd.between(1,4))
    }
...
function update() {
    ...
    if (game.input.activePointer.isDown && selectType != "pickingNode") {
        if (game.input.activePointer.duration < 140) {
            console.log("FLAG");
        ...
        if (selectType == "node" || selectType == "pickingNode") {
            selectedNode = -1
            for (var x = 0; x < nodes; x++) {
                if (node[x].input.pointerOver(0)) {
                    selectedNode = x
                    console.log("|S")
                    console.log("pointer over : " + x + "  =  " + node[x].input.pointerOver(0))
                }
            }
    ...
    if (win == true){
      winLabel = game.add.text(game.width/2 - 300, game.height/2 - 115, 'You Win', { font: '200px Arial', fill: '#fff' })  
    }
...
function render() {
    game.debug.text(("Tick: " + tick), 100, game.height  - 10);
