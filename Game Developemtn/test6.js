...
var htarget = [];
var hfire = [];
var desRate = 50; // Amount of time per decision (Ticks to (*3))
var hmove = 100; // Making next hmove mark
// Nodes for all Boids
var target = [];
var node = [];
...
function update() {
    ...
    if (game.input.activePointer.isDown && selectType != "pickingNode") {
        if (game.input.activePointer.duration < 40 && !captureOrigin) {
